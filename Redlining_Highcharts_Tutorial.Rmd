...
This script creates two JS files, zone_js and zone_js2.  Creating these files is not as complicated as it looks, but still there are many steps involved.  For those interested in creating multilevel visualizations, I recommend studying the code yourself.  The zone_js drilldown file in its final form can be found [here]().

For R to read these as javascript files, these need to be called inside the JS function from the htmlwidgets package.
